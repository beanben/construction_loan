...
class ProjectCosts:
    ...
    @classmethod
    def from_budget_csv(cls, csv_file_path: str) -> 'ProjectCosts':
    ...
    def _fundable_costs_cashflow(budget_df_init: pd.DataFrame) -> pd.DataFrame:
        ...
        cashflow_df = cashflow_df.resample(ProjectCosts.billing_period).sum().round(2)

        return cashflow_df

    @classmethod
    def from_cashflow_csv(cls, csv_file_path: str) -> 'ProjectCosts':
    ...
    def list_costs(self):
        # Summing the costs for each category/type/supplier
        # cost_breakdown = self.cashflow.sum().unstack(level=[0, 1, 2])
        cost_breakdown = self.cashflow.sum()

        # Adding a total row at the end
        cost_breakdown['Total'] = cost_breakdown.sum()
    ...
    def grouped_by_largest(self):
       ...
       # Create a new DataFrame to store the summarized data
        ...
        cashflow_df_summary.loc['Total'] = cashflow_df_summary.sum()

        return cashflow_df_summary
    
    @property
    def fundable_costs(self):
        return self.grouped_by_level("cost_category")["total"]
